...
## 進度（phased）
- **2026-06 全站電子寵物（已上線）**：常駐 `AppShell` 的浮動寵物 `components/pet/PetWidget.vue`，**預設關閉（opt-in）**，**沿畫面底部散步（mode A）**：左下登場、左右走動到邊緣轉身、偶爾發呆/睡覺（zzz）、避開右下聊天泡泡、`prefers-reduced-motion` 與手機自動不亂跑；點牠開小卡（名字/Lv/XP 進度條/設定/讓牠休息）並 +XP（有冷卻）。走路角色改用**側面造型** `components/pet/WalkingCreature.vue`（viewBox `0 0 140 116`，預設面向右、parent 用 `scaleX(facing)` 翻面＝往哪走面向哪；綁骨 group：4 腿/尾/耳/頭各有 `transform-box: view-box` 與自訂 pivot，CSS `b-*` class 驅動）。**行為狀態機**（PetWidget）：`walk/idle/sit/sleep/sniff/happy`，加權隨機 + 計時切換（`pickIdleBehavior`/`behaviorDuration`），走路才位移，摸摸觸發 `happy` 小跳後回 `walk`。前面那隻 `Creature.vue`（全身正面，viewBox `0 0 120 150`）保留當**肖像**用（PetView 動物挑選 tile、ProfileSetup）。散步 `min` 從側欄右緣起算[桌機 ≥1024 留 280px]。`PetBehavior` 型別由 `WalkingCreature` 的普通 `<script>` 區塊 export（`<script setup>` 不能 export）。**路線圖：全套**（使用者選；已完成側面朝向＋行為狀態機；未做：需求系統[飽食/精力/心情衰減、餵食/玩耍]、看向/跑向游標、成長進化、音效/粒子/配件商店/成就）。＋`data/animals`/`data/accessories`。狀態 `composables/usePet.ts`（module-scope 單例，localStorage 即時＋背景同步 `pet_state`、雲端優先、靜默降級）：`enabled/animal/name/accessory/level/xp/xpToNext/lastActiveDate`；心情由活躍度推導（今天有來=great、幾天沒來=tired）；XP 來源＝每日回訪 +15、摸摸 +3（冷卻）。側邊欄 `navGroups` 加大類「寵物」→ `/pet`（`PetView.vue`：開關、命名、選 5 種動物、配件依等級解鎖、成長說明）。後端 `com.lifedashboard.pet`：`pet_state` 表（per-user JSON）+ `GET/PUT /api/pet/state`（比照 english_state）。**使用者待辦：後端重新部署**（`ddl-auto` 自動建 `pet_state`）＋前端重新部署。**未做（可選）**：全站各模組行為餵 XP（目前只有回訪/摸摸）；與健康模組那隻 companion 統一（目前各自獨立）；頑皮模式（自由亂跑 B/跟滑鼠 C）。
