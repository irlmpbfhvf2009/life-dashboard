...
import { Menu, Search, Bell, LogOut, User, ChevronDown } from 'lucide-vue-next'
...
<template>
  <header class="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-ink-200 bg-ink-50/80 px-4 backdrop-blur-md lg:px-8">
    ...
    <button class="hidden w-full max-w-sm items-center gap-2 rounded-xl border border-ink-200 bg-white px-3.5 py-2 text-sm text-ink-400 shadow-card transition-colors hover:border-ink-300 sm:flex">
    ...
    <div class="ml-auto flex items-center gap-1.5">
      ...
      <div class="relative">
        ...
        <Transition name="menu">
          <div
            v-if="menuOpen"
            class="absolute right-0 top-12 w-56 overflow-hidden rounded-2xl border border-ink-200 bg-white p-1.5 shadow-pop"
          >
