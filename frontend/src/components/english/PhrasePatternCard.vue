...
<template>
  <div class="card p-6">
    <div class="mb-3 flex items-center justify-between">
      <span class="text-xs font-medium text-brand-600">{{ SCENARIO_LABELS[pattern.scenario] }}</span>
    ...
    <div class="mt-4 space-y-2">
      <p class="text-xs font-semibold text-ink-500">例句</p>
    ...
    <div v-if="pattern.commonMistakes.length" class="mt-4">
      <p class="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-amber-600"><AlertTriangle class="h-3.5 w-3.5" /> 常見錯誤</p>
      <ul class="space-y-1">
    ...
    <div class="mt-5 border-t border-ink-100 pt-4">
      ...
      <div class="flex gap-2">
        <input v-model="draft" type="text" class="input flex-1" placeholder="用這個句型造句…" @keydown.enter="check" />
        <button class="btn-primary btn-sm gap-1.5" :disabled="checking || !draft.trim()" @click="check">
          <Wand2 class="h-3.5 w-3.5" /> {{ checking ? '檢查中…' : 'AI 檢查' }}
        </button>
      </div>
