...
<template>
  <div class="card p-4">
    <div class="mb-2 flex items-center justify-between gap-2">
      <span class="badge badge-amber">{{ t('ec.mcat.' + mistake.category) }}</span>
    ...
    <div class="mt-3 flex items-center justify-between">
      <span class="text-xs text-ink-400">{{ t('ec.mistakes.recent', { date: formatDate(mistake.lastSeen) }) }}</span>
      <button
      ...
      >
        ...
        {{ queued ? t('ec.act.queued') : t('ec.act.addReview') }}
      </button>
    </div>
  </div>
</template>
